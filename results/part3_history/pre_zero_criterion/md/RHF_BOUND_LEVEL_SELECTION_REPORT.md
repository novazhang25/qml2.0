...
**Historical report:** its `E0_rel/E1_rel` values are signed energy gaps relative to dissociation. For explicit absolute `E_re + E_n` values, independent consistency checks, the complete corrected table and the replacement Part 3 command, see [PART3_CORRECTION.md](PART3_CORRECTION.md). The original scan evidence and this historical numerical record are preserved; replacement outputs use `results/bond_length_part3/`.

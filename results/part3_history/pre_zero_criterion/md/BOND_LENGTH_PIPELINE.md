...
**Part 3 replacement:** use `codes/bond_length_part3.py` for the explicit absolute vibrational-level analysis. See [the audit, corrected equations, complete saved-data table and run command](PART3_CORRECTION.md). It consumes the existing optimized equilibrium, force constant, effective mass and validated absolute plateau; Parts 1 and 2 do not run. The new code and tests were authored but not executed at the user's request.

The three-stage entry point documented below is retained as the legacy workflow. Its `E0_rel/E1_rel` fields are signed gaps to dissociation, not absolute energies. The replacement reports `E_total(n) = E_re + E_n`, compares it with absolute `E_diss`, and independently verifies `E_n < D_e` with `D_e = E_diss - E_re`.

...
**AO-selection history:** the descriptor counts, AO indices, and saved results
below document the legacy highest-principal-shell/chemical-core selection.
It retained H 1s, Li–F 2s/2p, and S 3s/3p uniformly by principal shell;
thus sulfur also lost 2s/2p. The revised final descriptor selection excludes
only heavy-atom 1s after the full-AO Löwdin transformation, changing only H2S
from 6 to 10 AOs/qubits. See [current AO selection](DESCRIPTOR.md) for verified
indices and dimensions. Historical tables and results below are preserved;
this distinction does not change `RMP2(mf, frozen=0)` or FCI settings.
